...
const SkillsList: React.FC = () => {
  ...
  const toggleItem = (category: string) => {
    setOpenItem((prev) => (prev === category ? null : category));
  };
        ...
        {Object.entries(skillsByCategory).map(([category, items]) => (
          <li key={category} className="w-full">
            <div
              ...
              className="w-full md:w-[400px] bg-[#1414149c] rounded-2xl text-left 
                         hover:bg-opacity-80 transition-all border border-[var(--white-icon-tr)] 
